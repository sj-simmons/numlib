...
import copy
from typing import Type, cast, Tuple, TypeVar, Union, Generic, Callable, Optional, Any, overload, Literal, Generator, Dict, List
...
class MontgomeryCurveMeta(type, Generic[F]):
    ...
    def __repr__(cls) -> str: ...
    order: int
    basepoint: MontgomeryCurve
    basepoint_order: int
    #co: Tuple[Optional[F]]

#class MontgomeryCurve(Generic[F], metaclass = EllipticCurve):
class MontgomeryCurve(EllipticCurve[F], metaclass = MontgomeryCurveMeta):
    def __init__(self, x: Union[int, F], y: Union[int, F, None] = None, z: Union[int, F, None] = None) -> None:
...
def Weierstrass(a: F, b: F, debug: bool = False) -> Type[WeierstrassCurve[F]]:
    ...
    class Meta(WeierstrassCurveMeta[F1]):
        ...
        disc = cast(F1, -16 * (4 * cast(F, a)**3 + 27 * cast(F ,b)**2))
        j = cast(F1, -110592 * cast(F, a)**3) / disc if disc != zero else None
    ...
    class WeierstrassCurve_(WeierstrassCurve[F1], metaclass=Meta):
        if debug:
            def __init__(self, x: Union[int, F1], y: Union[int, F1], z: Union[int, F1] = cast(F1, one)) -> None:
...
def affine(E: Type[EllipticCurve[FF]]) -> Generator[EllipticCurve[FF], None, None]:
    ...
    y2s: Dict[FF, List[FF]] = {}  # map all squares y2 in the field k to {y | y^2 = y2}
    fs: Dict[FF, List[FF]] = {}  # map all outputs fs = f(x), x in k, to {x | f(x) = fs}

    # build y2s and fs
    for x in coefcls:
        x2 = x**2
        y2s.setdefault(cast(FF, x2), []).append(cast(FF, x))
        fs.setdefault(cast(FF, x2 * x + a * x + b), []).append(cast(FF, x))
...
@overload
def affine2(E: Type[EllipticCurve[ZModP]]) -> Generator[EllipticCurve[ZModP], None, None]: ...
@overload
def affine2(E: Type[EllipticCurve[GF_ZModP]]) -> Generator[EllipticCurve[GF_ZModP], None, None]: ...

def affine2(E: Type[EllipticCurve[FF]]) -> Generator[EllipticCurve[FF], None, None]:
    ...
    for x in coefcls:
        fx = E.f(cast(FF, x))
        if fx ** ((q - 1) // 2) == 1:
            yield E(cast(FF, x), y=sqrt(fx, q=q, p=p))

def frobenious2(E: Type[EllipticCurve[FF]], n: int = 1, m: int = 1) -> Callable[[EllipticCurve[FF]], EllipticCurve[FF]]:
    ...
    Examples:
        ...
        >>> # A curve over GF(7,3) but with coeffients in Z/7:
...
def frobenious(
    E: EllipticCurve[FF], r: int) -> Callable[[EllipticCurve[FF]], EllipticCurve[FF]]:
