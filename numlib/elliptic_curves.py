...
def Weierstrass(a: F, b: F, debug: bool = False) -> Type[WeierstrassCurve[F]]:
    ...
    #aa = copy.copy(a)
    #if isinstance(aa, FPolynomial) and aa._degree > 0:
    #    if aa.x.find("(") < 0 or aa.x.find("[") < 0:
    #        aa.x = "(" + aa.x + ")"
    #bb = copy.copy(b)
    #if isinstance(bb, Polynomial) and bb._degree > 0:
    #    if bb.x.find("(") < 0 or bb.x.find("[") < 0:
    #        bb.x = "(" + bb.x + ")"
    #f_ = Polynomial((bb, aa, zero, one), "x -")
...
def Montgomery(a: F, b: F, debug: bool = False) -> Type[MontgomeryCurve[F]]:
    ...
    #f_ = Polynomial((zero, one, aa, one), "x -")
