...
from typing import Type, Union, TypeVar, Optional, Sequence, cast, overload, Callable
...
#  NOTE: Always multiply polynomials on the right by instance of Z_Mod_ (IS THIS STIL RELEVANT??)
...
def Pmod(monic: Polynomial[R]) -> Type[Pmod_]:
    ...
    class Pmod_(Polynomial, metaclass=Pmod__):
        def __init__(self, coeffs: Sequence[R], x=None, spaces=True, increasing=False):
...
def FPmod(fpoly: FPolynomial) -> Type[FPmod_]:
...
def GaloisField(p: int, r: int = 1, negatives=True, indet="t") -> Type[FPolynomial]:
    ...
    #    #return GF([0, 1], x)
    #    return GF(t, x)
    def indeterminant(indet: str = 'x', spaces: bool = True, increasing: bool = True) -> FPolynomial[FPolynomial[ZModP]]:
