...
def Zmod(n: int, mp=False, negatives=False, indet='x', spaces = False, increasing = False) -> Type[int]:
...
def FPmod(fpoly: FPolynomial) -> Type[FPolynomial]:
    ...
    Examples:
        ...
        Z/5[t]/<t^3+t^2+2>
        >>> GF
        Z/5[t] mod FPolynomial((2 + <5>, 0 + <5>, 1 + <5>, 1 + <5>))
        >>> t = GF()
        >>> t**3
        -t^2-2 + <t^3+t^2+2>
...
def GaloisField(p: int, r: int = 1, negatives = True, indet = 't') -> Type[FPolynomial]:
    ...
    Args:
        ...
        indet (str): a string specifying, e.g., the indeterminant (see
            examples).
    ...
    Examples:
        ...
        >>> GF = GaloisField(5, 3)
        >>> print(GF)  # elements are cubic polynomials over Z/5
        Z/5[t]/<t^3+t^2+2>
        ...
        all units), you end up doing say:
    ...
    PF = Zmodp(p, negatives = negatives, indet = 't', spaces = False, increasing = False)
    t = PF()
    ...
    class GF_FPmod_(FPolynomial, metaclass = FPmod__):
        ...
        #    if s[0] == "(" and s[-1] == ")":
        #        s = s[1:-1]
        #    return f"<{s}>" if self._degree is None  else f"{super().__str__()} + <{s}>"
